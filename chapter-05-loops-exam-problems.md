 ...
 * Инициализационен блок, в който се декларира променливата-брояч (**`i`**). С помощта на вградената
 функция (**`range`**) в python, определяме каква да бъде
 началната и крайната стойност на променливата-брояч.
 * Условие за повторение (**`i <= 11`**), изпълняващо се веднъж, преди всяка итерация на цикъла.
 * Обновяване на брояча – този код се изпълнява след всяка итерация.
...
Преди да продължим напред ще направим едно малко отклонение от настоящата тема, а именно ще споменем накратко, че в програмирането всяка променлива е от някакъв **тип данни**. В тази задача ще използваме както **цели числа** така и **дробни**. 
...
Декларираме си променливи **`p1_percentage`**, **`p2_percentage`** и т.н., в които ще се пазят процентите, както и **`cnt_p1`**, **`cnt_p2`** и т.н., в които ще пазим броя на числата от съответната група. В Python, когато декларираме променлива не оказваме какъв е нейният тип, както в някои други типизирани езици като C#, Java, C, C++ и т.н. Python сам преценява, какъв ще бъде типа на променливата в момента на компилирането на програмата. 
...
За да прочетем и разпределим всяко число в съответната му група, ще си послужим с **`for` цикъл** от **0** до **`n`** + 1 (броя на числата).     Всяка итерация на цикъла ще прочита и разпределя **едно единствено** число (**`current_number`**) в съответната му група. За да определим   дали едно число принадлежи към дадена група, **правим проверка в съответния ѝ диапазон**. Ако това е така - увеличаваме броя на числата в тази група (**`cnt_p1`**, **`cnt_p2`** и т.н.) с 1.  
...
Няма значение дали ще разделим на **100** (число тип **`int`**) или на **100.0** (число тип **`float`**), **делението** ще се извърши и в променливата ще се запази резултата от него. Например: **5 / 2 = 2.5**, а **5 / 2.0 = 2.5**. В **`python 3`**, няма значение дали ще се дели на цяло число или реално, ако резултата е реално число, то той ще се запише в променливата като число с плаваща запетая. Но в **`python 2.7`** ще бъде необходимо числата да се сведат до тип **`float`**, за да получим правилен резултат - реално число. Имайки това предвид, формулата за първата променлива ще изглежда така: 
...
В случая **`n = 3`**.
За цикъла имаме:
   -   	**`i = 0`** - прочитаме числото 1, което е по-малко от 200 и попада в първата група (**`p1`**), увеличаваме брояча на групата (**`cnt_p1`**) с 1.
   -   	**`i = 1`** – прочитаме числото 2, което отново попада в първата група (**`p1`**) и увеличаваме брояча ѝ (**`cnt_p1`**) отново с 1.
...
След прочитането на числата в група **`p1`** имаме 2 числа, а в **`p5`** имаме 1 число. В другите групи **нямаме числа**. Като приложим гореспоменатата формула, изчисляваме процентите на всяка група. Няма значение как ще умножим по **100**, или по **100.0** ще получим за група **`p1`** 66.67%, а за група **`p5`** – 33.33% (c дробна част). Но все пак да споменем отново, че това е валидно само за **`python 3`**.
...
В повечето скриптови езици, както и python не се грижим да определяме типа на променливите, които декларираме. Компилаторът сам преценява какви да бъдат те, след като се компилира скрипта. За годините на Лили (**`age`**) и единичната цена на играчката (**`present_price`**) по условие е дадено, че ще са **цели числа**. Затова ще използваме **вградента функция `int`** да конвентираме резултатът от въвеждането в тип цяло число. Когато се използва функция **`input()`**, резултатът въведен в конзолата винаги е от тип текст т.е. **`string`**, затова ако е необходимо той да бъде от друг тип, може да се използват **вградените в `python` функции** за целта. За цената на пералнята (**`price_of_washing_machine`**) знаем, че е **дробно число и избираме `float`**. В кода по-горе **декларираме** и **инициализираме** (присвояваме стойност) на променливите.
...
В някои програмни езици съществува конструкция, като **условния оператор (`?:`)** (наричан още тернарен оператор), тъй като записът е по-кратък. Синтаксисът му е следният: **`операнд1 ? операнд2 : операнд3`**. Първият операнд трябва да е от **булев тип** (т.е. да връща **`true/false`**). Ако **`операнд1`** върне стойност **`true`**, то ще се изпълни **`операнд2`**, а ако върне **`false`** – **`операнд3`**. В **`python`** също има такъв тернарен оператор, но той се записва чрез **`if-else`** конструкция. Синтаксисът му е следният: **`операнд1 if operand2 else operand3`** и работи така както и описаният по-горе принцип на работа на тернарните оператори. В нашия случай проверяваме дали **събраните пари** от Лили стигат за една пералня. Ако те са повече или равни на цената на пералнята, то проверката **`saved_money >= price_of_washing_machine`** ще върне **`true`** и ще се отпечата „**Yes! …**“, а ако е по-малко – резултатът ще е **`false`** и ще се отпечата “**No! …**”. Разбира се, вместо условния оператор, можем да използваме и **`if`** проверки.
...
Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1054#1](https://judge.softuni.bg/Contests/Practice/Index/1054#0).
...
Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1054#2](https://judge.softuni.bg/Contests/Practice/Index/1054#0).
...
Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1054#3](https://judge.softuni.bg/Contests/Practice/Index/1054#0).
...
Примерни имена на променливи може да са: **`n`** – брой на числата (който трябва да прочетем от конзолата) и **`divisible_by_2`**, **`divisible_by_3`**, **`divisible_by_4`** – помощни променливи, пазещи броя на числата от съответната група.
...
Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1054#4](https://judge.softuni.bg/Contests/Practice/Index/1054#0).
...
Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1054#5](https://judge.softuni.bg/Contests/Practice/Index/1054#0).
