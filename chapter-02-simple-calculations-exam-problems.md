...
**Placeholder** представлява израз, който ще бъде заменен с конкретна стойност при отпечатване. Функцията **`print(…)`** поддържа печатане на текст по шаблон, като разгледахме няколко възможни начина:
...
```
  ...
  * Втория начин са т.нар "f-strings", при които **пред** форматиращия низ трябва да долепим символа **`f`**, а аргументите изброяваме в самия низ, между **`{}`** скоби:
...
В случая може да пробваме със следното търсене: "[***Python get whole number part of double***](https://www.google.com/?q=c%23+get+whole+number+part+of+double)". Откриваме, че едната възможност е да използваме метода **`Math.Truncate(…)`**. Тъй като той работи с променливи от тип **`double`**, за броя редици и колони създаваме променливи също от този тип.
