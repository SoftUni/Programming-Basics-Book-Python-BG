 ...
 * **`round(…)`** - приема два параметър - първият е **числото, което искаме да закръглим**, а вторият - числото, което определя **с колко символа след десетичния знак да се извърши закръглянето** (това число винаги трябва да бъде цяло число). Закръглянето се извършва по основното правило за закръгляване - ако десетичната част е по-малка 5, закръглението е надолу и обратно, ако е по-голяма от 5 - нагоре:
...
```python
number_to_trunc = 2.63;
console.log(math.floor(number_to_trunc));
