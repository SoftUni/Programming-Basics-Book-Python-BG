...
Официалната **Python код конвенция** на PSF е публикувана в страницата "PEP 8 &mdash; Style Guide for Python Code" (https://www.python.org/dev/peps/pep-0008/) и в тази книга ще се ръководим основно от нея.

За форматиране на кода от PSF се препоръчва всяко ниво на влагане да бъде **4 празни полета навътре**, както е в примера по-долу.  **Езикът не позволява смесването на табове и интервали.**
...
Форматирането на кода е от изключително важно значение за функционирането му. Код, който не е форматиран правилно, дава грешка.
...
<<<<<<< HEAD
Когато бъде стартиран, кодът дава грешка и програмата спира работа:
...
За ваше удобство има **бързи клавиши в PyCharm**, за които ще обясним по-късно в настоящата глава, но засега ни интересуват една комбинация. Тя е за форматиране на **кода в целия документ**: [**CTRL + ALT + L**]. PyCharm по
=======

За ваше удобство има **бързи клавиши в PyCharm**, за които ще обясним по-късно в настоящата глава, но засега ни интересуват 2 конкретни комбинации. Едната комбинация е за форматиране на **кода в целия документ**, а другата комбинация - за форматиране на **част от кода**. Ако искаме да форматираме **целия файл**, то трябва да натиснем [**CTRL + K + D**]. В случай, че искаме да форматираме само **част от кода**, то ние трябва да **маркираме с мишката частта**, която искаме да форматираме, и да натиснем [**CTRL + K + F**].
>>>>>>> 5f7d324d6ec9b7f5b2ae3f918069f5ea1afe6175
...
<td>Въпреки че използването на главни букви в имената на променливите е разрешено, в Python това не се препоръчва и се счита за лош стил на именуване.</td>
</tr></table>
...
Бързи клавиши са **комбинации**, които ни предоставят възможността да извършваме някои действия **по-лесно и по-бързо**, като всяка среда за разработка на софтуер си има своите бързи клавиши, въпреки че повечето се повтарят. Сега ще разгледаме някои от **бързите клавиши** във **PyCharm**.
...
Получаваме предупреждение, че трябва да изберем контекст, т.е. в кои случаи да се показва нашият шаблон. Избираме **Define** и от появилото се мено слагаме отметка пред **Python**:
...
В случай, че искаме винаги да закръгляме надолу можем вместо **`round(…)`** да използваме друга функция - **`math.floor(…)`**, която винаги закръгля надолу, но също така винаги закръгля до цяло число. Например, ако имаме числото 5.99 и използваме **`math.floor(5.99)`**, ще получим числото **5**.

Можем и да направим точно обратното - винаги да закръгляме нагоре, използвайки функцията **`math.ceil(…)`**. Отново, ако имаме число примерно 5.11 и използваме **`math.ceil(5.11)`**, ще получим 6. Ето и няколко примера:

```python
import math
# Това дава възможност да използваме math.floor() и math.ceil()

number_to_floor = 5.99
print(math.floor(number_to_floor))
# Tова ще отпечата на конзолата 5

number_to_ceiling = 5.11
print(math.ceil(number_to_ceiling))
...
В случая след числото добавяме **`:.2f`**, което ще ограничи числото до 2 цифри след десетичния знак и ще работи като **`round(…)`**. Трябва да имаме предвид, че числото преди буквата **`f`** означава до колко цифри след десетичния знак да е закръглено числото (т.е. може да е примерно **`3f`** или **`5f`**). Не забравяйте и точката преди числото &ndash; тя е задължителна.
...
В **настоящата** глава се запознахме как **правилно** да **форматираме** и именуваме елементите на нашия **код**, някои **бързи клавиши** (shortcuts) за работа в PyCharm, **шаблони с код** (code snippets) и разгледахме как се **дебъгва код**.
