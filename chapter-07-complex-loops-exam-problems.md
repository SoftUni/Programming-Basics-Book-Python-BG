...
Нека декларираме и инициализираме **променливите**, които ще **съхраняват символите** на паролата: за **цифровите** символи - от тип **`int`** -  **`s1`**, **`s2`**, **`s3`**, **`s4`** и **`s5`**.

#### Извеждане на резултат

Необходимо е да вложим **пет** **`for`** цикъла един в друг, по един за всяка променлива. За да гарантираме условието последната цифра **s5** да бъде **по-голяма** от първите две, ще използваме вградената функция **`max(…)`**. 

![](/assets/chapter-7-2-images/01.stupid-password-generator-2.png)

**Знаете ли, че…?** 

* За удобство използваме директно ASCII стойността на символа **`а`** в декларациите на циклите - **`97`**:
...
* Или обратното да генерираме символ спрямо зададена стойност, използвайки вградена функция в Python **`chr(number)`**:
    
![](/assets/chapter-7-2-images/01.stupid-password-generator-5.png)
    
* Python разполага с богат набор от функции и методи за работа със стрингове. Потърсете допълнителна информация за функциите: **`str(number)`**, **`.lower()`**, **`.format(*args, **kwargs)`**, **`.swapcase()`**, **`.upper()`**.


### Тестване в Judge системата

Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1058#0](https://judge.softuni.bg/Contests/Practice/Index/1058#0).
...
Първо трябва да запишем **входното магическо число** в подходяща променлива.

![](/assets/chapter-7-2-images/02.magic-numbers-2.png)

След това ще започнем да разписваме **`while`** циклите.
- Ще инициализираме **първата цифра**: **`a = 0`**.
- Ще зададем **условие за всеки** цикъл: цифрата да бъде по-малка или равна на 9.
- В **началото** на всеки цикъл задаваме стойност на **следващата** цифра, в случая: **`b = 0`**. При вложените **`for`** цикли инициализираме променливите във вътрешните цикли при всяко увеличение на външните. Искаме да постигнем същото поведение и тук.
- В **края** на всеки цикъл ще **увеличаваме** съответната цифра с едно: **`a++`**, **`b++`**, **`c++`** и т.н..
...
Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1058#1](https://judge.softuni.bg/Contests/Practice/Index/1058#1).
...
Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1058#2](https://judge.softuni.bg/Contests/Practice/Index/1058#2).
...
Решете задачата самостоятелно използвайки наученото от предишните две. Спомнете си разликата между операторите за **целочислено деление ( __/__ )** и **деление с остатък ( __%__ )** в Python.

### Тестване в Judge системата

Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1058#3](https://judge.softuni.bg/Contests/Practice/Index/1058#3).
...
Тествайте решението си тук: [https://judge.softuni.bg/Contests/Practice/Index/1058#4](https://judge.softuni.bg/Contests/Practice/Index/1058#4).
