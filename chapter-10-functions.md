...
В настоящата глава ще се запознаем с **функции** и ще научим какво **представляват** те, както и кои са **базовите концепции** при работа с тях. Ще научим защо е **добра практика** да ги използваме, как да ги **дефинираме** и **извикваме**. Ще се запознаем с **параметри** и **връщана стойност на функция**, както и как да използваме тази връщана стойност. Накрая на главата, ще разгледаме **утвърдените практики** при използването на функции.

## Какво е функция?

До момента установихме, че при **писане** на програма, която решава дадена задача, ни **улеснява** това, че **разделяме** задачата на **части**. Всяка част отговаря за **дадено действие** и по този начин не само ни е **по-лесно** да решим задачата, но и значително се подобрява както **четимостта** на кода, така и проследяването за грешки.

В контекста на програмирането **функция** се нарича **именувана група от инструкции**, които изпълняват дадена фунционалност. Тази група от инструкции е логически отделена и има конкретно име, така че изпълнението на инструкциите в групата може да бъде стартирано чрез това име в хода на изпълнението на програмата. Стартирането на изпълнението на инструкциите във функцията се нарича **извикване на функцията** (на английски function call или invoking a function).   
...
Тази функция има задачата да отпечата заглавие, което представлява поредица от символа **`-`**. Поради тази причина името ѝ е **`print_header`**. Кръглите скоби **`( `** и **`)` винаги** следват името, независимо как сме именували функцията. Важно е името на функциите, с които работим, да описва действието, което извършват. По-късно в тази глава ще разгледаме още утвърдени практики за избиране на имена на функциите.

**Тялото** на функцията съдържа **програмния код** (инструкциите), който се намира на следващия ред след двуеточието и е изписано по-навътре (с индентация). Двуеточието **винаги** следва **декларацията** ѝ и след него поставяме кода, който решава проблема, описан от името на функцията. Тялото на функцията (наборът от инструкции) се изписва по-навътре, обикновено 4 интервала, които обособяват тялото като отделен блок инструкции, прилежащи към функцията.  
...
* **Име на функцията**. Името на функцията е **определено от нас** и следва ключовата дума `def`, като не забравяме, че то трябва да **описва действието**, което е изпълнявана от инструкциите в тялото ѝ. В примера името е **`calculate_square`**, което ни указва, че задачата на тази функция е да изчисли квадрата на някое число.
* **Списък с параметри**. Дефинира се между скобите **`(`** и **`)`**, които изписваме след името му. Тук изброяваме поредицата от **параметри**, които функцията ще използва. Може да присъства **само един** параметър, **няколко** такива или да е **празен** списък. Ако няма параметри, то записваме само скобите **`()`**. В конкретния пример декларираме един параметър с името **`num`**.
* **Двуеточие**. След затварящата скоба слагаме двучеточие `:`, което оказва, че започва тялото на функцията.
* **Имплементация (тяло)**. В тялото на функцията описваме **алгоритъма** (инструкциите), по който тя решава даден проблем, т.е. тялото съдържа кода (програмен блок), който реализира **логиката** на функцията. В показания пример изчисляваме квадрата на дадено число, а именно **`num * num`**. Тялото тук е записано на нов ред с индентация (няколко символа навътре).

При дефиниране на функции е важно да спазваме тази конкретна **последователност** на елементите.

Ако тялото на функцията се състои от **една инструкция**, то можем да го запишем на същия ред след двуеточието (или на отделен ред - по наш избор). Ако тялото се състои от **повече от една инструкция**, то трябва да го запишем на следващия ред, като инструкциите са записани с индентация (по-навътре от ключовата дума `def`, обикновено 4 знака `space`). Тази група от инструкции, записани по-навътре в кода, се нарича **програмен блок**.
...
<table>
<tr><td><img src="/assets/alert-icon.png" style="max-width:50px" /></td>
<td>Някои езици за програмиране (например C или C++) различават <b>декларирането</b> и <b>дефинирането</b> на функции. Декларирането на функция информира компилатора или интерпретатора, че функцията със съответното име и параметри същестува, без да съдържат имплементацията (тялото) ѝ. Дефиницията съдържа имплементацията (тялото) на функцията. В езика за програмиране Python това различаване не съществува и функциите винаги биват дефинирани, т.е. при създаването на функция винаги трябва да предоставим нейната имплементация.</td>
</tr>
</table>
...
Дадена функция може да бъде извикана от **няколко места** в нашата програма и повече от един път. Важно е да знаем, че в езика за програмиране Python, ако една функция е дефинирана някъде в програмата, то тя може да бъде извиквана само след самата ѝ дефиниция.
...
## Функции с параметри (сложни функции)
...
**Дефинираме** функцията и **списъка** ѝ с **параметри**, след което пишем тялото, което тя ще изпълнява.
...
При **декларирането на параметри** можем да използваме **различни** типове данни, като всеки един параметър трябва да има **име**. Важно е да отбележим, че при последващото извикване на функцията, трябва да предаваме **стойности** за параметрите по **реда**, в който са **декларирани** самите те. 

Нека разгледаме друг пример за дефиниция на функция, която има няколко параметъра от различен тип:

![](/assets/chapter-10-images/chapter-10-6-print-student.png)
...
Първото извикване на функцията ще използва параметрите `start=5`, `end=10`. Второто извикване - параметрите `start=0`, `end=15`. Третото извикване - параметрите `start=0`, `end=100`. А четвъртото извикване ще използва параметрите `start=35` и `end=45`.
...
![](/assets/chapter-10-images/chapter-10-9-triangle-1.png)
...
До момента разгледахме функции, които извършват дадено действие, например отпечатване на даден текст, число или фигура на конзолата. Освен този тип функции, съществуват и такива, които могат да **връщат** някакъв **резултат**, който да бъде използван на мястото, където извикване функцията. Именно тези функции ще разгледаме в следващите редове.

### Оператор return

За да върнем резултат от функция, използваме оператора **`return`**. Той трябва да бъде **използван в тялото** на функцията и указва на програмата да **спре изпълнението** ѝ и да **върне** на извиквача на функцията определена **стойност**, която се определя от израза след въпросния оператор **`return`**.

В примера по-долу имаме функция, която чете две имена от конзолата, съединява ги и ги връща като резултат:

![](/assets/chapter-10-images/chapter-10-12-return-operator-1.png)

Операторът **`return`** може да бъде използван и във функции, които не връщат резултат. Изпълнението му води до това функцията да спре изпълнението си, без да връща никаква стойност, а след оператора не трябва да има израз. В този случай употребата на **`return`** е единствено за прекратяване на изпълнението на функцията.

Възможно е операторът **`return`** да бъде използван на повече от едно място в тялото на функцията.
...
![](/assets/chapter-10-images/chapter-10-12-return-operator-2.png)
...
![](/assets/chapter-10-images/chapter-10-12-return-operator-3.png)

### Типове на връщаната от функцията стойност

До сега разглеждахме примери, в които имплементацията на функцията **не** връща резултат, а изпълнява определено действие.

![](/assets/chapter-10-images/chapter-10-11-return-example-1.png)

Функциите могат също да връщат резултати, които да бъдат използвани от извиквача:


![](/assets/chapter-10-images/chapter-10-11-return-example-2.png)

Важно е да отбележим, че **резултатът**, който се връща от функцията, може да бъде от **различен тип** - низ, число, число с плаваща запетая и т.н. 
...
![](/assets/chapter-10-images/chapter-10-13-return-value-1.png)

Вторият е резултатът да бъде използван **в израз**:

![](/assets/chapter-10-images/chapter-10-13-return-value-2.png)

Третият е да **предадем** резултата от работата на функцията към **друга функция** чрез параметър:

![](/assets/chapter-10-images/chapter-10-13-return-value-3.png)
...
![](/assets/chapter-10-images/chapter-10-14-triangle-area-1.png)
...
![](/assets/chapter-10-images/chapter-10-14-triangle-area-2.png)
...
![](/assets/chapter-10-images/chapter-10-15-power-numbers.png)

След като сме направили нужните изчисления, ни остава да разпечатаме резултата.
...
За целта при използване на оператора **`return`** в езика за програмиране Python отделяме стойностите, които искаме да върнем, със запетая. Следната функция приема за параметри две целочислени числа (**`x`** и **`y`**) и **връща две стойности** - резултата от целочислено деление на двете числа и остатъка от делението им:

![](/assets/chapter-10-images/chapter-10-16-return-tuple.png)
...
![](/assets/chapter-10-images/chapter-10-17-overloading-1.png)

В този пример сигнатурата на функцията е нейното име (**`print_name`**), както и нейният параметър (**`name`**).

Ако в програмата ни има **функции с еднакви имена**, но с **различни сигнатури**, то казваме, че имаме **варианти на функции (function или method overloading)**. Езикът за програмиране Python не позволява дефинирането на две функции с едно и също име. 

### Варианти на функции в Python

Различни варианти за извикване на една функция в езика за програмиране Python може да бъде постигнато чрез използването на незадължителни параметри, или по-конкретно - чрез предоставянето на стойности по подразбиране за дадени параметри.

Нека разгледаме примера от по-рано за функция с няколко незадължителни параметъра:
...
![](/assets/chapter-10-images/chapter-10-18-nested-functions-1.png)
...
Вложените функции имат достъп до променливите, които се използват в съдържащата ги функция. В примера по-горе променливата `pi` може да бъде използвана от тялото на функцията `circle_diameter`.
...
В езика за програмиране Python е прието имената на функциите да се изписват с **малки букви**, като отделните думи се отделят с **долна черта**. Добра практика е името на функцията да е съставено от глагол или от двойка: глагол и съществително име. 
...
При писането на функции трябва да внимаваме да спазваме коректна **индентация** (отместване по-навътре на блокове от кода). В езикът за програмиране Python грешната **индентация** много често автоматично води до грешна програмата или такава, която не може да бъде изпълнена.

Пример за **правилно** форматиран Python код:

![](/assets/chapter-10-images/chapter-10-19-best-practices-1.png)

Пример за **некоректно** форматиран Python код (затова и последният ред е подчертан в червено):

![](/assets/chapter-10-images/chapter-10-19-best-practices-2.png)
...
Дефинирайте функция **`print_name(name)`** и я имплементирайте, след което в главната програма прочетете от конзолата низ (име на човек) и извикайте функцията като ѝ подадете прочетеното име.
...
Дефинирайте функция **`get_min(a, b)`** и я имплементирайте, след което я извикайте от главната програма както е показано по-долу. За да намерите минимума на три числа, намерете първо минимума на първите две от тях и след това минимума на резултата и третото число:

![](/assets/chapter-10-images/chapter-10-20-exercises-1.png)
...
![](/assets/chapter-10-images/chapter-10-20-exercises-2.png)
...
![](/assets/chapter-10-images/chapter-10-20-exercises-4.png)
...
В **`read_and_process_message()`** прочетете типа съобщение от конзолата и според прочетения тип прочетете останалите данни (още един два или три реда). След това извикайте съответния метод за печатане на съответния тип съобщение.
...
#### Насоки и подсказки
На променливата от тип **`string`**, в която ще се пази стойността на резултата, ще присвоим първоначална стойност **`""`** (празен низ). Трябва да се завърти цикъл **`n`** пъти, като на всяка итерация към променливата, в която пазим стойността на резултата, ще прибавяме криптирания символ. 
