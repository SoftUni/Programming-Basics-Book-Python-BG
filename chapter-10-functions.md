...
<td>В обектно-ориентираното програмиране (което не е предмет на тази книга) функциите, които са част от класове, се наричат <b>методи</b>. В някои езици за програмиране функциите се наричат още <b>процедури</b> или <b>подпрограми</b>. </td>
</tr>
</table>
...
* **Списък с параметри**. Дефинира се между скобите **`(`** и **`)`**, които изписваме след името му. Тук изброяваме поредицата от **параметри**, които функцията ще използва. Може да присъства **само един** параметър, **няколко** такива или да е **празен** списък. Ако няма параметри, то записваме само скобите **`()`**. В конкретния пример декларираме параметъра **`num`**.
* **Двуеточие**. След затварящата скоба слагаме двучеточие `:`, което оказва, че започва тялото на функцията.
* **Имплементация (тяло)**. В тялото на функцията описваме **алгоритъма** (инструкциите), по който тя решава даден проблем, т.е. тялото съдържа кода (програмен блок), който реализира **логиката** на функцията. В показания пример изчисляваме лицето на квадрат, а именно **`num * num`**. Тялото тук е записано на нов ред с индентация (няколко символа навътре).
...
След това **извикваме** функцията като ѝ **предаваме конкретни стойности** за параметрите:
...
Нека разгледаме примера за дефиниция на функция, която има няколко параметъра:
...
За да получим резултат от функцията, използваме оператора **`return`**. Той трябва да бъде **използван в тялото** на функцията и указва на програмата да **спре изпълнението** му и да **върне** на извиквача на функцията определена **стойност**, която се определя от израза след въпросния оператор **`return`**.
...
Операторът **`return`** може да бъде използван и във функции, които не връщат резултат. Тогава самиата функция ще спре изпълнението си, без да връща никаква стойност, а след оператора не трябва да има израз, който да бъде върнат. В този случай употребата на **`return`** е единствено за прекратяване на изпълнението на функцията.
...
За целта при използване на оператора `return` отделяме стойностите, които искаме да върнем, със запетая. Следната функция приема за параметри две целочислени числа (**`x`** и **`y`**) и **връща две стойности** - резултата от целочислено деление на двете числа и остатъка от делението им:
...
В много езици за програмиране една и съща функция може да е декларирана в **няколко варианта** с еднакво име и различни параметри. Това е известно с термина “**function overloading**” (или “**method overloading**”). Сега нека разгледаме как можем да декларираме такива функции.
...
В този пример сигнатурата на функцията е нейното име (**`print`**), както и нейният параметър (**`text`**).

Ако в програмата ни има **функции с еднакви имена**, но с **различни сигнатури**, то казваме, че имаме **варианти на функции (function или method overloading)**.
