...
![](/assets/chapter-6-2-images/01.nested-for.png)

Запознахме се и с **оператора `*`**, който дава възможност да се печата **даден символ определен** от нас **брой** пъти:

![](/assets/chapter-6-2-images/02.print-me.png)
...
Декларираме си и втора **променлива**, в която ще пазим **стойността** на частта **между двете кули**. Междинната част на покрива е с размер  **`2 * n - 2 * col_size - 4`**.
...
Виждаме също така, че горното крило на пеперудата е с размер **`n - 2`**, затова можем да направим цикъл, който да се повтаря **`half_row_size`** пъти.
...
За да направим **тялото на пеперудата** е необходимо да отпечатаме на конзолата точно **един** ред. Структурата на тялото е с начало **`(празно място)`**, среда **`@`** и край **`(празно място)`**, като празните места се повтарят **`n - 1`** пъти.

![](/assets/chapter-6-2-images/02.Butterfly-05.png)

Остава да отпечатаме на конзолата и долното крило, което е противоположно на горното крило, но правим същите итерации.
...
**Долната част** на фигурата, в която знака се **смалява**, можем да направим като отново създадем **цикъл**, който да се завърти **`n`** брой пъти. Структурата на един ред е начало **`.`** + **`\\`**, среда **`_`** и край **`//`** + **`.`**. Броят на **точките** при първата итерация на цикъла трябва да е 0 и на всяка следваща да се **увеличава** с едно. Следователно можем да кажем, че големината на **точките в долната част от фигурата** е равна на **`i`**. За да работи нашата програма правилно, трябва на всяка итерация от **цикъла** да **намаляваме** броя на **`_`** с **2**.
...
Тъй като оператора **`*`** не може да съедини символ 0 пъти в низ, **цикъла**, който ще направим, трябва да се завърти **`n - 2`** пъти и отделно да отпечатаме последния ред от фигурата. На всяка итерация **`outer_dots`** се увеличава с 1, а **`inner_dots`** намалява с 2.
...
За да можем да използваме отново създадените **променливи** при чертането на дръжката на брадвата, трябва да намалим **средните тирета** с 1,а **тиретата от дясно** и **тиретата от ляво** да увеличим с 1. Ще използваме и нова **променлива** за височината на дръжката, която ще бъде със стойност **`n - 2`**.

![](/assets/chapter-6-2-images/05.Axe-03.png)

**Дръжката на брадвата** можем да нарисуваме, като завъртим цикъл, който се повтаря **`n - 2`** пъти и точно тук ще използваме декларираната и инициализирана променлива за нейната големина. От примерите можем да разберем, каква е нейната структура.

![](/assets/chapter-6-2-images/05.Axe-04.png)

Следващата част от фигурата се оформя като **глава на брадвата** и се включва още едно разклонение, за целта е необходимо да намалим **тиретата от ляво** с 1.
...
![](/assets/chapter-6-2-images/05.Axe-07.png)
