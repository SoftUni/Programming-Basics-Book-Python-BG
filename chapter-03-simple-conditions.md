...
![](/assets/chapter-3-1-images/ComparingNumbers.png)
...
![](assets/chapter-3-1-images/ComparingStrings.png)
...
![](assets/chapter-3-1-images/ExcellentGrade.png)
...
![](assets/chapter-3-1-images/ExcellentOrNot.png)
...
Чрез **табулация** създаваме блок от код, чрез който може да се изпълняват група от команди. Когато имаме код в **if, elif, else** и
искаме да изпълним поредица от операции ги поставяме в блок след условието.

<table><tr><td><img src="/assets/alert-icon.png" style="max-width:50px" /></td>
<td>Добра практика е, <strong>да ползваме табулация или четири space bar-а</strong>, понеже това прави кода ни по-четим и по-подреден и
по този начин избягваме грешки по време на изпълнение на кода.</td>
</tr></table>

Ето един пример за лоша практика:

![](/assets/chapter-3-1-images/WrongConventionExample.png)

Изпълнението на горния код ще изведе следния резултат на конзолата:

![](/assets/chapter-3-1-images/ResultFromWrongCode.png)

С правилно конструиране на блоковете:

![](/assets/chapter-3-1-images/RightConventionExample.png)

На конзолата ще бъде отпечатано следното:

![](/assets/chapter-3-1-images/ResultFromRightCode.png)
...
Първата ни задача е да **прочетем** двете числа. След което, чрез проста **`if-else`** конструкция, в съчетание с **оператора за по-голямо** (**`>`**), да направим проверка. Кода е недовършен умишлено, за да изпробваме наученото до момента.
...
В примера по-долу, на последния ред, на който се опитваме да отпечатаме променливата **`my_name`**, която е дефинирана в **`else` конструкцията**, ще получим **грешка**, защото така и не сме влезли в **`else`** за да я инициализираме. Но при отпечатването на променливата **`can_drive`** нямаме проблем, защото сме влезли в **`if`** и сме я инициализирали. Както забелязвате обаче променливите **`can_drive`** и **`my_name`** за оцветени в жълто. Това е предупреждение от **PyCharm**, че е възможно да получим грешка. Затова е най-добре да внимаваме с това къде инициализираме променливите.

![](/assets/chapter-3-1-images/VariableScope.png)
...
![](/assets/chapter-3-1-images/BonusPoints.png)

Ето как би могло да изглежда решението на задачата в действие:

![](/assets/chapter-3-1-images/BonusPintsOutput.png)
...
![](/assets/chapter-3-1-images/SumSeconds.png)
...
![](/assets/chapter-3-1-images/MetricConverter.png)
...
![](/assets/chapter-3-1-images/Debugging-1.png)
![](/assets/chapter-3-1-images/Debugging-2.png)

### Дебъгване в PyCharm

Чрез натискане на [**Shift + F9**], стартираме програмата в **debug режим**. Преминаваме към **следващия ред** с [**F7**]:

![](/assets/chapter-3-1-images/Debugging.png)

Чрез [**Ctrl + F8**] създаваме стопери – така наречените **breakpoints**, до които можем да стигнем директно използвайки [**Shift + F9**] при стартирането на програмата.
