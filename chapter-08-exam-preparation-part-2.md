...
<td>Типът данни за реални числа с десетична точност в <b>Python</b> е 64-битовият тип <b><code>Decimal</code></b>. Той има <b>точност</b> по подразбиране <b>28</b> десетични знака. <b>Минималната</b> му стойност е <b>-9×10^28</b>, а <b>максималната</b> е <b>+9×10^28</b>. Стойността му по подразбиране е <b>0</b>. За да го използваме, първо трябва да го импортираме. Това става със написването на следния код в началото на нашата програма:<br/>
<code><b>from</b> decimal <b>import</b> Decimal</code><br/>
Ако стандартната прецизност от 28 знака не ние е достатъчна, можем да я променим: <br/>
<code>getcontext().prec = 30</code>
<br/>
...
**Изминалото време** (в часове) пресмятаме като **разделим времето на 60** (колото са минутите в един час). **Изминатото разстояние** намираме като **умножим началната скорост с изминалото време** (в часове). След това променяме скоростта, като я увеличаваме с **10%** по условие. Пресмятането на **процентите**, както и следващите изминати **разстояния**, извършваме по следния начин:
...
От условието виждаме, че **първо** ще ни бъде подаден **броя** на студентите, а едва **след това оценките** им. По тази причина **първо** в една променлива от тип **`int`** ще прочетем **броя** на студентите. За да прочетем и обработим самите оценки, ще използваме **`for`** цикъл. Стойността на променливата **`int`** ще бъде **крайната** стойност на променливата **`i`** от цикъла. По този начин **всика** итерация на цикъла ще прочете по **една оценка**. 
...
За **начертаването** на **динамичната** или променлива част от фигурката ще използваме **цикъл**. Размерът на цикъла ще бъде от **0** до **широчината**, която имаме по условие, а именно **`4 * n + 1`**. Тъй като тази формула ще използваме на **няколко места** в кода, е добра практика да я изнесем в **отделна променлива**. Преди изпълнението на цикъла би следвало да **заделим променливи** за **броя** на отделните символи, които участват в динамичната част: **точки** и **тирета**. Чрез изучаване на примерите можем да изведем формули и за **стартовите стойности** на тези променливи. Първоначално **тиретата** са **0**, но броят на **точките** ясно се вижда, че можем да получим като от **общата широчина** извадим **3** (броя символи, които изграждат върха на коледната шапка) и след това **разделим на 2**, тъй като броят точки от двете страни на шапката е еднакъв.
...
За последната задача имаме по условие входни данни на **3 реда**, които са представени от по един символ от **ASCII таблицата** ([http://www.asciitable.com/](http://www.asciitable.com/)). Бихме могли да използваме вече **дефинирана функция** в езика Python, като преобразуваме входните данни в тип данни **`char`** по следния начин:
...
Най-лесният и удачен начин е да използваме **цикъл**, като преминем през **всички символи** и принтираме тези, които са **различни** от **буквата**, която трябва да пропуснем. Едно от предимствата на езика Python, е че имаме възможност да използваме различен тип данни за циклична променлива:
